...
const type = route.params.navbar as string
...
const getSortParams = (): {
    ...
    switch (section) {
        case "highly-ranking":
            sortBy.push({ key: 'average_rating', order: 'desc' })
            break
        case 'most-liked':
            sortBy.push({key: 'like_count', order: 'desc'})
            break
        case 'new-arrivals': 
    ...
    if (selectedColor.value) options.color = selectedColor.value
    if (selectedSize.value) options.size = selectedSize.value
...
watch(
    () => [route.params.navbar, route.query.section],
    () => {
        curPage.value = 1
