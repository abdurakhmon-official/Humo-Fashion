import { classOf, isArray, isBoolean, isEmpty, isNil, nameOf, objectKeys } from "@tsed/core";
import { getPropertiesStores, JsonEntityStore, JsonParameterStore, JsonSchema, VendorKeys } from "@tsed/schema";
...
export class JsonDeserializer extends JsonMapperCompiler {
    ...
    getPropertyFiller(propertyStore, id, groups, formatOpts) {
        ...
        if (propertyStore.isCollection) {
            if (propertyStore.itemSchema.getGenericOf()) {
                ...
                return (writer) => {
                    return writer.callMapper(nameOf(propertyStore.collectionType), varKey(key), `id: '${nestedMapper.id}'`, `generics: schemes['${schemaId}'].getGenericOf()`, formatOpts);
                };
            }
    ...
    mapGenericsOptions(label, inputGenerics) {
