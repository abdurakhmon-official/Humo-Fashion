...
import { Application, urlencoded, Response, json, Request } from 'express'
import hpp from 'hpp'
...
export class Server {
    ...
    $afterRoutesInit() {
        ...
        console.info(`====== STAGE: ${config.state} ======`)
        ...
        this.app.use((err: any, req: Request, res: Response, next: Function) => {
            ...
            if (err && !res.headersSent) {
                if ((err as any) instanceof ZodError) {
                    res.status(400).send({
                        success: false,
                        _message: err.errors.length ? err.errors.map((e: any) => `${e.path.join()} - ${e._message}`).join('/n') : err._message
                    })
